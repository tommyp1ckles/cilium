					...
					t.NewAction(s, fmt.Sprintf("ping-%s-%d", ipFam, i), &client, ep, ipFam).Run(func(a *check.Action) {
						sout, serr, err := client.K8sClient.ExecInPodWithStderr(
							...
							[]string{
								"python",
								"-c",
								scapyImport + fmt.Sprintf("; resp = sr1(IPv6(dst='%s', src=get_if_addr6('eth0'))/TCP(dport=1234)/('*' * %d), timeout=5); print(resp)",
									extEndpoint, t.Context().Params().ExternalPMTUEndpointMaxMTU),
							},
						)
						if err != nil {
					...
					t.NewAction(s, fmt.Sprintf("ping-%s-%d", ipFam, i), &client, ep, ipFam).Run(func(a *check.Action) {
						sout, serr, err := client.K8sClient.ExecInPodWithStderr(
							...
							[]string{
								"python",
								"-c",
								scapyImport + fmt.Sprintf("; resp = sr1(IP(dst='%s', src=get_if_addr('eth0'), flags='DF',frag=0)/TCP(dport=1234)/('*' * %d), timeout=5); print(resp)",
									extEndpoint, t.Context().Params().ExternalPMTUEndpointMaxMTU),
							},
						)
						if err != nil {
