...
func newAllNodeManager(lc hive.Lifecycle, ipCache *ipcache.IPCache, nodeMetrics *nodeMetrics, hr cell.HealthReporter) (NodeManager, error) {
	mngr, err := New(option.Config, ipCache, nodeMetrics, hr)
	if err != nil {
		return nil, err
	}
	lc.Append(mngr)
