...
func (n *linuxNodeHandler) deallocateIDForNode(oldNode *nodeTypes.Node) error {
	...
	if err := n.deallocateNodeIDLocked(nodeID); err != nil {
		errs = errors.Join(errs, fmt.Errorf("failed to deallocate node ID %d: %w", nodeID, err))
	}
	return errs
}
	...
	for ip, id := range n.nodeIDsByIPs {
		if nodeID == id {
			if err := n.unmapNodeID(ip); err != nil {
				...
				errs = errors.Join(errs, fmt.Errorf("failed to unmap IP %q with node ID %q: %w", ip, nodeID, err))
			}
		}
	}
