...
func (n *linuxNodeHandler) allocateIDForNode(node *nodeTypes.Node) (uint16, error) {
	...
	if nodeID == 0 {
		...
		if nodeID == uint16(idpool.NoID) {
			...
			acc = errors.Join(acc, fmt.Errorf("no available node ID %q", node.Name))
		} else {
			log.WithFields(logrus.Fields{
				logfields.NodeID:   nodeID,
...
func (n *linuxNodeHandler) deallocateIDForNode(oldNode *nodeTypes.Node) error {
	...
	for _, addr := range oldNode.IPAddresses {
		...
		if nodeID != id {
			...
			acc = errors.Join(acc, fmt.Errorf("found two node IDs (%d and %d) for the same node", id, nodeID))
		}
	}

	if err := n.deallocateNodeIDLocked(nodeID); err != nil {
		acc = errors.Join(acc, fmt.Errorf("deallocate node ID %d: %w", nodeID, err))
	}
	return acc
}
	...
	for ip, id := range n.nodeIDsByIPs {
		if nodeID == id {
			if err := n.unmapNodeID(ip); err != nil {
				...
				acc = errors.Join(acc, fmt.Errorf("unmap IP %q with node ID %q: %w", ip, nodeID, err))
			}
		}
	}
