...
func updateTunnelMapping(oldCIDR, newCIDR cmtypes.PrefixCluster, oldIP, newIP net.IP,
	...
	if cidrNodeMappingUpdateRequired(oldCIDR, newCIDR, oldIP, newIP, oldEncryptKey, newEncryptKey) {
		...
		if err := tunnel.TunnelMap().SetTunnelEndpoint(newEncryptKey, nodeID, newCIDR.AddrCluster(), newIP); err != nil {
			...
			errs = errors.Join(errs,
				fmt.Errorf("failed to update tunnel endpoint map (nodeID: %d, prefix: %s, nodeIP: %s): %w", nodeID, newCIDR.AddrCluster(), newIP, err))
		}
	}
...
func (n *linuxNodeHandler) enableIPsec(newNode *nodeTypes.Node) error {
	...
	if newNode.IsLocal() {
		if err := n.replaceHostRules(); err != nil {
			errs = errors.Join(errs, fmt.Errorf("failed to replace host rules: %w", err))
		}
	}
	...
	return errors.Join(errs, n.enableIPsecIPv4(newNode, zeroMark), n.enableIPsecIPv6(newNode, zeroMark))
}
...
func (n *linuxNodeHandler) enableIPsecIPv4(newNode *nodeTypes.Node, zeroMark bool) error {
	...
	if newNode.IsLocal() {
		...
		if err != nil {
			errs = errors.Join(errs, fmt.Errorf("failed to allocate ID for node: %w", err))
		}
		...
		if n.subnetEncryption() {
			...
			if err := n.replaceNodeIPSecOutRoute(new4Net); err != nil {
				errs = errors.Join(errs, fmt.Errorf("failed to replace node ipsec OUT route: %w", err))
			}
			spi, err = ipsec.UpsertIPsecEndpoint(localCIDR, remoteCIDR, localIP, remoteIP, remoteNodeID, ipsec.IPSecDirOut, false)
			errs = errors.Join(errs, upsertIPsecLog(err, "out IPv4", localCIDR, remoteCIDR, spi))
		}
	}
	return errs
}
...
func (n *linuxNodeHandler) enableIPsecIPv6(newNode *nodeTypes.Node, zeroMark bool) error {
	...
	if newNode.IsLocal() {
		...
		if n.subnetEncryption() {
			...
			spi, err := ipsec.UpsertIPsecEndpoint(localCIDR, remoteCIDR, localIP, remoteIP, remoteNodeID, ipsec.IPSecDirOut, false)
			errs = errors.Join(errs, upsertIPsecLog(err, "out IPv6", localCIDR, remoteCIDR, spi))
		}
	}
	return errs
}
...
func (n *linuxNodeHandler) nodeUpdate(oldNode, newNode *nodeTypes.Node, firstAddition bool) error {
	...
	if n.nodeConfig.EnableIPSec && !n.nodeConfig.EncryptNode {
		if err := n.enableIPsec(newNode); err != nil {
			errs = errors.Join(errs, fmt.Errorf("failed to enable IPSec: %w", err))
		}
	...
	if n.nodeConfig.EnableEncapsulation {
		...
		if err != nil {
			errs = errors.Join(errs, fmt.Errorf("failed to enable encapsulation: allocating ID: %w", err))
		}
		...
		if err := updateTunnelMapping(oldPrefixCluster4, newPrefixCluster4, oldIP4, newIP4, firstAddition, n.nodeConfig.EnableIPv4, oldKey, newKey, nodeID); err != nil {
			errs = errors.Join(errs, fmt.Errorf("failed to enable encapsulation: updating tunnel mapping for ipv4: %w", err))
		}
		// Not a typo, the IPv4 host IP is used to build the IPv6 overlay
		if err := updateTunnelMapping(oldPrefixCluster6, newPrefixCluster6, oldIP4, newIP4, firstAddition, n.nodeConfig.EnableIPv6, oldKey, newKey, nodeID); err != nil {
			errs = errors.Join(errs, fmt.Errorf("failed to enable encapsulation: updating tunnel mapping for ipv6: %w", err))
		}
