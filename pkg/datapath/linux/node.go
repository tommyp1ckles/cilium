...
func upsertIPsecLog(err error, spec string, loc, rem *net.IPNet, spi uint8, accs ...*error) {
	...
	if err != nil {
		...
		for _, acc := range accs {
			*acc = errors.Join(*acc, fmt.Errorf("create endpoint %s (local_ip=%s, rem=%s, spi=%d): %w",
				spec,
				loc.String(),
				rem.String(),
				spi, err))
...
func (n *linuxNodeHandler) encryptNode(newNode *nodeTypes.Node) error {
	...
	if n.nodeConfig.EnableIPv4 {
		...
		if newNode.IsLocal() {
			...
			acc = errors.Join(acc, n.replaceNodeIPSecInRoute(ipsecLocal))
			...
			if remoteIPv4 := newNode.GetNodeIP(false); remoteIPv4 != nil {
				...
				acc = errors.Join(acc, n.replaceNodeExternalIPSecOutRoute(ipsecRemote))
			...
			if remoteIPv4 != nil {
				...
				acc = errors.Join(acc, n.replaceNodeExternalIPSecOutRoute(ipsecRemoteRoute))
	...
	if n.nodeConfig.EnableIPv6 {
		...
		if newNode.IsLocal() {
			...
			if remoteIPv6 != nil {
				...
				acc = errors.Join(n.replaceNodeExternalIPSecOutRoute(ipsecRemoteRoute))
...
func (n *linuxNodeHandler) enableIPsec(newNode *nodeTypes.Node) error {
	...
	if newNode.IsLocal() {
		if err := n.replaceHostRules(); err != nil {
			acc = errors.Join(acc, fmt.Errorf("replace host rules: %w", err))
		}
	}
	...
	acc = errors.Join(acc, n.enableIPsecIPv4(newNode, zeroMark), n.enableIPsecIPv6(newNode, zeroMark))
	return acc
}

func (n *linuxNodeHandler) enableIPsecIPv4(newNode *nodeTypes.Node, zeroMark bool) error {
	var spi uint8
	var acc error
	...
	if newNode.IsLocal() {
		acc = errors.Join(acc, n.replaceNodeIPSecInRoute(new4Net))
		...
		if n.subnetEncryption() {
			...
			if err != nil {
				...
				acc = errors.Join(acc, fmt.Errorf("getting local ipv4 for ipsec link: %w", err))
			}
			...
			for _, cidr := range n.nodeConfig.IPv4PodSubnets {
				...
				upsertIPsecLog(err, "in CiliumInternalIPv4", wildcardCIDR, cidr, spi, &acc)

				spi, err = ipsec.UpsertIPsecEndpoint(wildcardCIDR, cidr, localNodeInternalIP, wildcardIP, 0, ipsec.IPSecDirIn, zeroMark)
				upsertIPsecLog(err, "in NodeInternalIPv4", wildcardCIDR, cidr, spi, &acc)
			}
		} else {
		...
		if n.subnetEncryption() {
			...
			if err := n.replaceNodeIPSecOutRoute(new4Net); err != nil {
				acc = errors.Join(acc, fmt.Errorf("replacing node ipsec out route: %w", err))
			}
...
func (n *linuxNodeHandler) enableIPsecIPv6(newNode *nodeTypes.Node, zeroMark bool) error {
	var acc error
	var spi uint8
	...
	if err != nil {
		acc = errors.Join(acc, fmt.Errorf("default drop policy IPv6: %w", err))
	}
	upsertIPsecLog(err, "default-drop IPv6", wildcardCIDR, wildcardCIDR, spi)

	if newNode.IsLocal() {
		acc = errors.Join(acc, n.replaceNodeIPSecInRoute(new6Net))
		...
		if n.subnetEncryption() {
			...
			if err != nil {
				...
				acc = errors.Join(acc, fmt.Errorf("get local ipv6 for ipsec link: %w", err))
			}

			for _, cidr := range n.nodeConfig.IPv6PodSubnets {
				spi, err := ipsec.UpsertIPsecEndpoint(wildcardCIDR, cidr, localIP, wildcardIP, 0, ipsec.IPSecDirIn, zeroMark)
				upsertIPsecLog(err, "in CiliumInternalIPv6", wildcardCIDR, cidr, spi, &acc)

				spi, err = ipsec.UpsertIPsecEndpoint(wildcardCIDR, cidr, localNodeInternalIP, wildcardIP, 0, ipsec.IPSecDirIn, zeroMark)
				upsertIPsecLog(err, "in NodeInternalIPv6", wildcardCIDR, cidr, spi, &acc)
			}
		} else {
		...
		if n.subnetEncryption() {
			...
			if !option.Config.UseCiliumInternalIPForIPsec {
				...
				if err != nil {
					log.WithError(err).Error("Failed to get local IPv6 for IPsec configuration")
					acc = errors.Join(acc, fmt.Errorf("getting local ipv6 for ipsec link: %w", err))
				}
			...
			if err := n.replaceNodeIPSecOutRoute(new6Net); err != nil {
				acc = errors.Join(acc, fmt.Errorf("replace node ipsec out route: %w", err))
			}
			spi, err := ipsec.UpsertIPsecEndpoint(localCIDR, remoteCIDR, localIP, remoteIP, remoteNodeID, ipsec.IPSecDirOut, false)
...
func (n *linuxNodeHandler) nodeUpdate(oldNode, newNode *nodeTypes.Node, firstAddition bool) error {
	...
	if n.nodeConfig.EnableIPSec && !n.nodeConfig.EncryptNode {
		if err := n.enableIPsec(newNode); err != nil {
			acc = errors.Join(acc, fmt.Errorf("enable IPSec: %w", err))
		}
	...
	if n.nodeConfig.EnableIPSec && n.nodeConfig.EncryptNode && !n.subnetEncryption() {
		if err := n.encryptNode(newNode); err != nil {
			acc = errors.Join(acc, fmt.Errorf("encrypt node: %w", err))
		}
	}
	...
	if n.nodeConfig.EnableEncapsulation {
		...
		encapAcc = errors.Join(encapAcc, err)
		...
		encapAcc = errors.Join(encapAcc, updateTunnelMapping(oldPrefixCluster4, newPrefixCluster4, oldIP4, newIP4, firstAddition, n.nodeConfig.EnableIPv4, oldKey, newKey, nodeID))
		// Not a typo, the IPv4 host IP is used to build the IPv6 overlay
		encapAcc = errors.Join(encapAcc, updateTunnelMapping(oldPrefixCluster6, newPrefixCluster6, oldIP4, newIP4, firstAddition, n.nodeConfig.EnableIPv6, oldKey, newKey, nodeID))
...
func (n *linuxNodeHandler) replaceNodeExternalIPSecOutRoute(ip *net.IPNet) error {
	...
	if err := route.Upsert(n.createNodeExternalIPSecOutRoute(ip, true)); err != nil {
		...
		acc = errors.Join(acc, fmt.Errorf("replace ipsec default route OUT: %w", err))
	}
	if err := route.Upsert(n.createNodeExternalIPSecOutRoute(ip, false)); err != nil {
		log.WithError(err).WithField(logfields.CIDR, ip).Error("Unable to replace the IPSec route OUT the host routing table")
		acc = errors.Join(acc, fmt.Errorf("replace ipsec host route OUT: %w", err))
	}
	return acc
}
...
func (n *linuxNodeHandler) replaceNodeIPSecInRoute(ip *net.IPNet) error {
	...
	if err := route.Upsert(n.createNodeIPSecInRoute(ip)); err != nil {
		...
		return fmt.Errorf("replace ipsec host route IN: %w", err)
	}
	return nil
}
